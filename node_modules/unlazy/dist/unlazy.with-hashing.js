const _t = "data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7", ot = typeof window > "u", pt = !ot && "loading" in HTMLImageElement.prototype, bt = !ot && (!("onscroll" in window) || /(gle|ing|ro)bot|crawl|spider/i.test(navigator.userAgent));
...
function ht(t) {
  let { PI: e, min: n, max: r, cos: c, round: o } = Math, f = t[0] | t[1] << 8 | t[2] << 16, s = t[3] | t[4] << 8, a = (f & 63) / 63, p = (f >> 6 & 63) / 31.5 - 1, d = (f >> 12 & 63) / 31.5 - 1, u = (f >> 18 & 31) / 31, i = f >> 23, _ = (s >> 3 & 63) / 63, G = (s >> 9 & 63) / 63, g = s >> 15, I = r(3, g ? i ? 5 : 7 : s & 7), y = r(3, g ? s & 7 : i ? 5 : 7), w = i ? (t[5] & 15) / 15 : 1, T = (t[5] >> 4) / 15, v = i ? 6 : 5, m = 0, h = (B, L, D) => {
    let S = [];
    for (let C = 0; C < L; C++)
      for (let z = C ? 0 : 1; z * L < B * (L - C); z++)
        S.push(((t[v + (m >> 1)] >> ((m++ & 1) << 2) & 15) / 7.5 - 1) * D);
    return S;
  }, E = h(I, y, u), $ = h(3, 3, _ * 1.25), H = h(3, 3, G * 1.25), x = i && h(5, 5, T), N = gt(t), M = o(N > 1 ? 32 : 32 * N), F = o(N > 1 ? 32 / N : 32), U = new Uint8Array(M * F * 4), q = [], Z = [];
  for (let B = 0, L = 0; B < F; B++)
    for (let D = 0; D < M; D++, L += 4) {
      let S = a, C = p, z = d, J = w;
      ...
      for (let l = 0, A = 0; l < y; l++)
        for (let b = l ? 0 : 1, P = Z[l] * 2; b * y < I * (y - l); b++, A++)
          S += E[A] * q[b] * P;
      ...
      U[L] = r(0, 255 * n(1, tt)), U[L + 1] = r(0, 255 * n(1, At)), U[L + 2] = r(0, 255 * n(1, K)), U[L + 3] = r(0, 255 * n(1, J));
    }
  return { w: M, h: F, rgba: U };
}
function gt(t) {
...
function ct(t, e, n) {
  ...
  for (let d = 0, u = 0, i = r - 1; d < e; d++, i += r - 1)
    for (o.push(d + 1 < e ? 0 : 1, r & 255, r >> 8, ~r & 255, r >> 8 ^ 255, 0), a = (a + s) % 65521; u < i; u++) {
      const _ = n[u] & 255;
      o.push(_), s = (s + _) % 65521, a = (a + s) % 65521;
    }
  o.push(
    a >> 8,
    a & 255,
  ...
  for (let [d, u] of [[12, 29], [37, 41 + c]]) {
    let i = -1;
    for (let _ = d; _ < u; _++)
      i ^= o[_], i = i >>> 4 ^ f[i & 15], i = i >>> 4 ^ f[i & 15];
    i = ~i, o[u++] = i >>> 24, o[u++] = i >> 16 & 255, o[u++] = i >> 8 & 255, o[u++] = i & 255;
  }
...
const mt = "0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz#$%*+,-.:;=?@[]^_{|}~", Y = (t, e, n) => {
  ...
  return r;
}, at = Math.pow, R = Math.PI, xt = R * 2, it = 3294.6, lt = 269.025, Ct = (t) => t > 10.31475 ? at(t / lt + 0.052132, 2.4) : t / it, k = (t) => ~~(t > 1227e-8 ? lt * at(t, 0.416666) - 13.025 : t * it + 1), O = (t) => (t < 0 ? -1 : 1) * t * t, rt = (t) => {
  for (t += R / 2; t > R; )
    t -= xt;
  const e = 1.27323954 * t - 0.405284735 * O(t);
  return 0.225 * (O(e) - e) + e;
};
function It(t) {
  const e = Y(t, 2, 6);
...
function St(t, e, n, r) {
  const c = Y(t, 0, 1), o = c % 9 + 1, f = ~~(c / 9) + 1, s = o * f;
  let a = 0, p = 0, d = 0, u = 0, i = 0, _ = 0, G = 0, g = 0, I = 0, y = 0, w = 0, T = 0, v = 0, m = 0;
  const h = (Y(t, 1, 2) + 1) / 13446 * (r | 1), E = new Float64Array(s * 3), $ = It(t);
  for (a = 0; a < 3; a++)
    E[a] = Ct($[a]);
  for (a = 1; a < s; a++)
    m = Y(t, 4 + a * 2, 6 + a * 2), E[a * 3] = O(~~(m / (19 * 19)) - 9) * h, E[a * 3 + 1] = O(~~(m / 19) % 19 - 9) * h, E[a * 3 + 2] = O(m % 19 - 9) * h;
  const H = e * 4, x = new Uint8ClampedArray(H * n);
  for (u = 0; u < n; u++)
    for (T = R * u / n, d = 0; d < e; d++) {
      for (i = 0, _ = 0, G = 0, v = R * d / e, p = 0; p < f; p++)
        for (I = rt(T * p), a = 0; a < o; a++)
          g = rt(v * a) * I, y = (a + p * o) * 3, i += E[y] * g, _ += E[y + 1] * g, G += E[y + 2] * g;
      w = 4 * d + u * H, x[w] = k(i), x[w + 1] = k(_), x[w + 2] = k(G), x[w + 3] = 255;
    }
  return x;
}
function zt(t, {
  ...
  const { width: r, height: c } = yt(e, n), o = St(t, r, c);
  return ct(r, c, o);
}
function ft(t = 'img[loading="lazy"]', {
  ...
  for (const s of st(t)) {
    ...
    if (s.src || (s.src = _t), s.complete && s.naturalWidth > 0) {
      Q(s, o);
      continue;
    }
    const p = () => Q(s, o);
...
function Gt(t = 'img[data-sizes="auto"], source[data-sizes="auto"]') {
  for (const e of st(t))
    j(e);
}
function Q(t, e) {
  ...
  ut(t), r && (n.srcset = r), c && (n.src = c), n.addEventListener("load", () => {
    X(t), V(t), e == null || e(t);
  });
}
function Ut({
  ...
  if (e)
    try {
      ...
      (typeof __UNLAZY_LOGGING__ > "u" || __UNLAZY_LOGGING__) && console.error(`Error generating ${n} placeholder:`, o);
    }
}
...
function j(t, e) {
  ...
  if (n && (t.sizes = `${n}px`), ((r = t.parentElement) == null ? void 0 : r.tagName.toLowerCase()) === "picture" && !(e != null && e.skipChildren) && [...t.parentElement.getElementsByTagName("source")].forEach(
    ...
    if (!W.has(t)) {
      const c = Lt(() => j(t), 500), o = new ResizeObserver(c);
      W.set(t, o), o.observe(t);
    }
    return () => {
      const c = W.get(t);
      c && (c.disconnect(), W.delete(t));
    };
  }
}
function V(t) {
